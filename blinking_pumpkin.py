...
def pump_pulse(pumpkin):
    pumpkin.sides.left.bottom.pulse(
        5, 0.5, 1
    )  # Fade in for 5 seconds, fade out for .5 seconds and do this once.
    pumpkin.eyes.right.on()
    pumpkin.eyes.right.blink(0.5, 0.5, 0, 0, 12)
    ...
    sleep(0.5)
    pumpkin.eyes.right.off()
    pumpkin.eyes.left.on()
    pumpkin.eyes.left.blink(0.5, 0.5, 0, 0, 12)
